...
sys.path.append("table_transformer_markdown/detr")
...
def process_single_argument(arg):
    target, pred_logits, pred_bboxes, mode = arg
    return eval_tsr_sample(target, pred_logits, pred_bboxes, mode)
...
def dar_con(true_adjacencies, pred_adjacencies):
    ...
    fscore, precision, recall = grits.compute_fscore(
        num_true_positives, len(true_adjacencies), len(pred_adjacencies)
    )
...
def compute_metrics(
    ...
    true_text_grid = np.array(
        grits.cells_to_grid(true_cells, key="cell_text"), dtype=object
    )
    ...
    pred_text_grid = np.array(
        grits.cells_to_grid(pred_cells, key="cell_text"), dtype=object
    )
    ...
    ) = grits_top(true_relspan_grid, pred_relspan_grid)
    ...
    if mode == "grits-all":
        ...
        (
            ...
            metrics["grits_rawloc_upper_bound"],
        ) = grits_loc(true_cell_dilatedbbox_grid, pred_cell_dilatedbbox_grid)
        ...
        ) = dar_con_original(true_cells, pred_cells)

        # Compute updated DAR (directed adjacency relations) metric
        (
            metrics["dar_recall_con"],
            metrics["dar_precision_con"],
            metrics["dar_con"],
        ) = dar_con_new(true_cells, pred_cells)
...
def print_metrics_summary(metrics_summary, all=False):
    ...
    for table_type in ["simple", "complex", "all"]:
        ...
        print(
            "Results on {} tables ({} total):".format(table_type, metrics["num_tables"])
        )
...
def visualize(args, target, pred_logits, pred_bboxes):
    ...
    for bbox, label, score in zip(pred_bboxes, pred_labels, pred_scores):
        if (
            (args.data_type == "structure" and not label > 5)
            or (args.data_type == "detection" and not label > 1)
            and score > 0.5
        ):
            color, alpha, linewidth, hatch = get_bbox_decorations(args.data_type, label)
            # Fill
...
def evaluate(args, model, criterion, postprocessors, data_loader, base_ds, device):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_every, header):
        ...
        if args.debug:
            for target, pred_logits, pred_boxes in zip(
                targets, outputs["pred_logits"], outputs["pred_boxes"]
            ):
        ...
        if args.data_type == "structure":
            ...
            if batch_num % args.eval_step == 0 or batch_num == num_batches:
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=args.eval_pool_size
                ) as executor:
                    metrics = list(executor.map(process_single_argument, arguments))
                tsr_metrics += metrics
                pred_logits_collection = []
                pred_bboxes_collection = []
                targets_collection = []
...
def eval_coco(
    args, model, criterion, postprocessors, data_loader_test, dataset_test, device
):
    """
    ...
    print(
        "COCO metrics summary: AP50: {:.3f}, AP75: {:.3f}, AP: {:.3f}, AR: {:.3f}".format(
