...
sys.path.append("table_transformer_markdown/src")
...
def get_args():
    ...
    parser.add_argument(
        "--structure_config_path", help="Filepath to the structure model config file"
    )
    parser.add_argument(
        "--structure_model_path", help="The path to the structure model"
    )
    parser.add_argument(
        "--detection_config_path", help="Filepath to the detection model config file"
    )
    parser.add_argument(
        "--detection_model_path", help="The path to the detection model"
    )
    ...
    parser.add_argument(
        "--visualize", "-z", action="store_true", help="Visualize output"
    )
    parser.add_argument(
        "--crop_padding",
        type=int,
...
class TableExtractionPipeline(object):
        ...
        if not det_config_path is None:
            ...
            if not det_model_path is None:
                self.det_model.load_state_dict(
                    torch.load(det_model_path, map_location=torch.device(det_device))
                )
    ...
    def detect(
        self, img, tokens=None, out_objects=True, out_crops=False, crop_padding=10
    ):
        out_formats = {}
        ...
        if out_crops:
            tables_crops = objects_to_crops(
                img, tokens, objects, self.det_class_thresholds, padding=crop_padding
            )
    ...
    def extract(
        ...
        for table in cropped_tables:
            ...
            extracted_table = self.recognize(
                img,
                tokens=tokens,
...
def main():
    ...
    pipe = TableExtractionPipeline(
        det_device=args.detection_device,
    ...
    for count, img_file in enumerate(img_files):
        ...
        if args.mode == "recognize":
            extracted_table = pipe.recognize(
                img,
                tokens,
